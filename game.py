...
class Display():
    ...
    def __init__(self, config):
        ...
        self.hero = pygame.transform.scale(
            pygame.image.load('ressource/MacGyver.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.guardian = pygame.transform.scale(
            pygame.image.load('ressource/Gardien.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.needle = pygame.transform.scale(
            pygame.image.load('ressource/aiguille.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.ether = pygame.transform.scale(
            pygame.image.load('ressource/ether.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.tube = pygame.transform.scale(
            pygame.image.load('ressource/tube_plastique.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.syringe = pygame.transform.scale(
            pygame.image.load('ressource/seringue.png'),
            (config.inventory_sprite, config.inventory_sprite)
        )
        self.wall = pygame.transform.scale(
            pygame.image.load('ressource/wall.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.path = pygame.transform.scale(
            pygame.image.load('ressource/path.png'),
            (config.sprite_height, config.sprite_width)
        )
        self.window = pygame.display.set_mode(config.window)

    def __convert_pos_to_pixel(self, pos, config):
        """Converti l'indice de position d'un objet
        en coordonnées (x, y) exprimé en pixel
        """
    ...
    def draw_window(self, config):
        ...
        text = font.render(
            'Usez des objets trouvés sur votre chemin', 1, (250, 250, 250))
        pixel_pos = self.__convert_pos_to_pixel(config.instruction_two, config)
        self.window.blit(text, pixel_pos)
        text = font.render(
            'pour passer outre le guardien de la sortie.', 1, (250, 250, 250))
        pixel_pos = self.__convert_pos_to_pixel(
            config.instruction_three, config)
        self.window.blit(text, pixel_pos)
    ...
    def draw_final(self, config):
        if config.win:
            ...
            font = pygame.font.SysFont('arial', 35, True)
            text = font.render(
                'Le guardien vous a attrapé !', 1, (250, 250, 250))
            pixel_pos = self.__convert_pos_to_pixel(config.final, config)
            self.window.blit(text, pixel_pos)
            pygame.display.update()
            pygame.time.wait(3000)
